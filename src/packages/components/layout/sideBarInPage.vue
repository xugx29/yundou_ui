  ...
  <div class="slideBarNav">
    <div class="leftNav">
      <div class="avatar">
        ...
        <div style="min-width:92px;padding-top:60px; position: absolute; top:0; left:10px;z-index:10000" @mouseenter="showDropdown2 = true" @mouseleave="showDropdown2 = false">
      ...
      <ul class="navList">
        <li class="topLevelLi" :class="{active: currentTopLevelName == item.name}" v-for="(item, index) in nav" :key="index" @mouseenter="setNavDataByHover(item.child, item.name)" @mouseleave="clearNavData">
          <i class="navIcon" :class="currentTopLevelName == item.name ? item.iconClass + '-hover' : item.iconClass"></i><span v-if="Object.keys(item.child).length == 0"><router-link :to="{path: item.url}">{{item.name}}</router-link></span>
